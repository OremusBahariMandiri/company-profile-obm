                        ...
                        @if ($activity->photo)
                            <div class="text-center">
                                ...
                                <img src="{{ \App\Helpers\StorageHelper::getStorageUrl($activity->photo) }}"
                                    alt="{{ $activity->title }}" class="img-fluid rounded border shadow-sm"
                        ...
                        @if ($activity->photo)
                            <div class="info-item mb-3">
                                ...
                                <ul class="list-unstyled mb-0">
                                    <li><small class="text-muted">Ukuran: {{ number_format($fileSize / 1024, 2) }}
                                            KB</small></li>
                                    @if (file_exists($fullPath) && function_exists('getimagesize'))
                                        ...
                                        @endphp
                                        @if ($imageInfo)
                                            <li><small class="text-muted">Dimensi: {{ $imageInfo[0] }}x{{ $imageInfo[1] }}
                                                    pixel</small></li>
                                        @endif
                                    @endif
                                </ul>
            ...
            <div class="card-body">
                ...
                <div class="website-preview border rounded p-3" style="background-color: #f8f9fa;">
                    <div class="activity-gallery-preview">
                        <div class="row">
                            <div class="col-md-6">
                                <div class="gallery-item-preview position-relative"
                                    ...
                                    @if ($activity->photo)
                                        ...
                                        {{-- AFTER: StorageHelper --}}
                                        <img src="{{ \App\Helpers\StorageHelper::getStorageUrl($activity->photo) }}"
                                            alt="{{ $activity->title }}" class="w-100 h-100" style="object-fit: cover;">
                    ...
                    <div class="mt-3 text-center">
                        <small class="text-muted">
                            ...
                            Preview: Aktivitas ini akan muncul di tab
                            "{{ $categories[$activity->category] ?? $activity->category }}" di halaman website
                        </small>
                    </div>
                </div>
            </div>
