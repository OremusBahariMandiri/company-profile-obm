                ...
                <form action="{{ route('activities.update', $activity) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="row mb-4">
                        ...
                        <div class="col-md-8 mb-3">
                            <label for="title" class="form-label">Judul Aktivitas <span
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <select class="form-select @error('category') is-invalid @enderror" id="category"
                                ...
                                @foreach ($categories as $key => $value)
                                    <option value="{{ $key }}"
                                        {{ old('category', $activity->category) == $key ? 'selected' : '' }}>
                                        {{ $value }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="row mb-4">
                            ...
                            <div class="col-12 mb-3">
                                <div class="current-photo text-center">
                                    ...
                                    {{-- AFTER: StorageHelper --}}
                                    <img src="{{ \App\Helpers\StorageHelper::getStorageUrl($activity->photo) }}"
                                        alt="Current Photo" class="rounded border"
                    ...
                    <div class="row mb-4">
                        ...
                        <div class="col-12 mb-3">
                            ...
                            <input type="file" class="form-control @error('photo') is-invalid @enderror" id="photo"
                                name="photo" accept="image/*" {{ !$activity->photo ? 'required' : '' }}>
                            <div class="form-text">
                                {{ $activity->photo ? 'Biarkan kosong jika tidak ingin mengganti foto. ' : '' }}
                                Maksimal 5MB. Format: JPG, JPEG, PNG, GIF. Disarankan ukuran 800x600 pixel untuk hasil
                                optimal.
                            </div>
                            @error('photo')
                                <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    <div class="row mb-4">
                        <div class="col-12">
                            ...
                            <div class="alert alert-info">
                                ...
                                <span
                                    class="badge bg-primary ms-1">{{ App\Http\Controllers\Admin\OurActivityController::getCategoryName($activity->category) }}</span>
                            </div>
                                ...
                                <div class="row">
                                    ...
                                    @foreach ($categories as $key => $value)
                                        <div class="col-md-2 mb-3">
                                            <div
                                                class="card border-{{ $categoryColors[$key] ?? 'secondary' }} h-100 {{ $activity->category == $key ? 'border-2 shadow' : '' }}">
                                                <div class="card-body text-center p-2">
                                                    <i
                                                        class="fas {{ $categoryIcons[$key] ?? 'fa-folder' }} fa-2x text-{{ $categoryColors[$key] ?? 'secondary' }} mb-2"></i>
            ...
            if (categorySelect) {
                categorySelect.addEventListener('change', function() {
                    ...
                    categoryCards.forEach(card => {
                        ...
                        if (selectedCategory && card.querySelector('.text-muted').textContent ===
                            selectedCategory) {
                            card.classList.add('border-2', 'shadow');
                            const cardBody = card.querySelector('.card-body');
                            if (cardBody && !card.querySelector('.badge')) {
                                cardBody.innerHTML +=
                                    '<div class="mt-2"><span class="badge bg-success">Dipilih</span></div>';
                            }
                        }
                    });
                });
